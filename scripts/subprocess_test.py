...
async def test_gpt_image_calls():
    ...
    image_path = os.path.join(dependencies_folder, f"{random_word}.png")
    print(dependencies_folder)

    try:
        path = await generate_gpt_image(random_word, image_path, definition)
        print("GPT image generated successfully at %s!", path) 
    except Exception as e:
        print(f"Error generating DALL-E image for word '%s': %s", random_word, e)   
# command = [
...
# test_mp4_creation()
# asyncio.run(test_dalle_calls())
asyncio.run(test_mp4_creation())